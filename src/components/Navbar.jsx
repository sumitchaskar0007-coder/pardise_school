...
const Navbar = () => {
  ...
  const [isUdanOpen, setIsUdanOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const udanDropdownRef = useRef(null);
  ...
  useEffect(() => {
    const closeUdanDropdown = (event) => {
      if (
        event.key === "Escape" ||
        (udanDropdownRef.current &&
          !udanDropdownRef.current.contains(event.target))
      ) {
        setIsUdanOpen(false);
      }
    };

    document.addEventListener("mousedown", closeUdanDropdown);
    document.addEventListener("keydown", closeUdanDropdown);
    return () => {
      document.removeEventListener("mousedown", closeUdanDropdown);
      document.removeEventListener("keydown", closeUdanDropdown);
    };
  }, []);
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 py-2 sm:py-3 flex items-center justify-between gap-2 sm:gap-3">
        ...
        {/* UDAN BOOKS */}
        <div ref={udanDropdownRef} className="hidden lg:block relative flex-shrink-0">
          <button
            type="button"
            onClick={() => setIsUdanOpen((open) => !open)}
            aria-expanded={isUdanOpen}
            aria-haspopup="menu"
            className="flex items-center gap-2 rounded-md bg-navy-700 px-4 py-2 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-navy-800 focus:outline-none focus:ring-2 focus:ring-navy-700 focus:ring-offset-2"
          >
            Udan
            <span
              aria-hidden="true"
              className={`text-xs transition-transform ${isUdanOpen ? "rotate-180" : ""}`}
            >
              ▼
            </span>
          </button>
          ...
          {isUdanOpen && (
            <div
              role="menu"
              className="absolute right-0 top-full z-[60] mt-2 w-36 overflow-hidden rounded-md border border-gray-200 bg-white py-1 shadow-xl"
            >
              {udanBooks.map((book) => (
                <a
                  key={book.path}
                  ...
                  role="menuitem"
                  onClick={() => setIsUdanOpen(false)}
                  className="block px-4 py-2 text-sm font-medium text-navy-800 transition-colors hover:bg-navy-50 focus:bg-navy-50 focus:outline-none"
                >
                  {book.label}
                </a>
              ))}
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="bg-navy-50 p-3 border-b border-navy-200">
              <details className="mb-3 rounded-md border border-navy-200 bg-white">
                <summary className="cursor-pointer px-3 py-2 text-sm font-semibold text-navy-800">
                  Udan
                </summary>
                <div className="grid grid-cols-3 border-t border-navy-200 p-2">
                  {udanBooks.map((book) => (
                    <a
                      key={book.path}
                      href={book.path}
                      ...
                      className="rounded px-2 py-2 text-center text-xs font-semibold text-navy-800 hover:bg-navy-50"
                    >
                      {book.label}
                    </a>
                  ))}
